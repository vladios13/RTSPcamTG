...
`lang` — язык Web UI, по умолчанию `ru`. Переключается в `/config` («Интерфейс» → «Язык») и применяется сразу после сохранения.
...
Telegram-команды: `/ustop` — пауза детекции до `/ustart`, `/ustop 30m` — пауза на время (`s`/`m`/`h`/`d`, не больше 24h) с авто-возвратом и сообщением в `tg_chat`, `/ustart` — возобновить, `/status` — аптайм, счётчики и свежесть кадров по камерам.
